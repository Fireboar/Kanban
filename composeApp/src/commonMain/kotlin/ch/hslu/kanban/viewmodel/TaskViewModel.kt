...
class TaskViewModel (
    ...
    fun addTask(task: Task) {
        viewModelScope.launch {
            ...
            if (success) {
                syncViewModel.setSyncMessage(
                    "'${task.title}' erfolgreich hinzugefügt und synchronisiert.",
                    true
                )
            } else {
                syncViewModel.setSyncMessage(
                    "'${task.title}' konnte nicht auf den Server hochgeladen werden.",
                    false
                )
            }
            loadTasks()
    ...
    fun updateTask(task: Task){
        viewModelScope.launch {
            ...
            if(success) {
                syncViewModel.setSyncMessage(
                    "'${task.title}' erfolgreich aktualisiert.",
                    true)
            } else {
                syncViewModel.setSyncMessage(
                    "'${task.title}' konnte nicht synchronisiert werden.",
                    false)
            }
            loadTasks()
        }
    ...
    fun deleteTask(task: Task) {
        viewModelScope.launch {
            ...
            if(success){
                syncViewModel.setSyncMessage(
                    "'${task.title}' erfolgreich gelöscht und synchronisiert.",
                    true
                )
            } else {
                syncViewModel.setSyncMessage(
                    "'${task.title}' konnte nicht vom Server gelöscht werden.",
                    false
                )
            }
            loadTasks()
    ...
    fun postTasks() {
        viewModelScope.launch {
            ...
            if(success){
                syncViewModel.setSyncMessage(
                    "Tasks wurden auf den Server gepostet.",
                    true
                )
            } else {
                syncViewModel.setSyncMessage(
                    "Fehler beim Posten der Tasks.",
                    false
                )
            }
            loadTasks()
    ...
    fun pullTasks() {
        viewModelScope.launch {
            ...
            if(success){
                syncViewModel.setSyncMessage(
                    "Tasks vom Server geladen und lokal synchronisiert.",
                    true
                )
            } else {
                syncViewModel.setSyncMessage(
    ...
    fun mergeTasks() {
        viewModelScope.launch {
            ...
            if(success){
                syncViewModel.setSyncMessage(
                    "Server- und lokale Tasks wurden zusammengeführt.",
                    true
                )
            } else {
                syncViewModel.setSyncMessage(
                    "Fehler beim Mergen der Tasks.",
                    false
                )
            }
            loadTasks()
